    ...
    if request.method == 'GET':
        if current_user.is_authenticated:
            ...
            if current_user.pro is None:
                #utilisateur                 
                infos = c.execute("select prenom, email, temps, budget, maison, appartement, id_utilisateur FROM  utilisateur where email=?", (current_user.id,)).fetchone()    
                ...
                #adresse
                infos_adresse = c.execute("select nb, rue, ville, code_postal from adresse inner join utilisateur on adresse.id_adresse=utilisateur.id_adresse where email=?", (current_user.id,)).fetchone()
                id_user=infos[6]
        ...
        new_prenom = request.form['pre']
        new_email = request.form['em']
        new_nb = request.form['nombre']
        new_rue = request.form['street']
        new_ville = request.form['vil']
        new_code_postal = request.form['cp']
        new_budget = request.form.get('prix')
        ...
        if infos_user is not None :
            ...
            if infos_adresse is None:
                ...
                c.execute("UPDATE utilisateur SET email=? and budget=? and prenom=? and id_adresse=? where id_utilisateur=?", (new_email, new_budget, new_prenom, id_adresse, id_current,))
                conn.commit()
